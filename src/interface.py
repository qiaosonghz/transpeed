...
def convert_args_and_invoke(args, eval_func, shape, num_simd_lanes, bit_width, bandwidth, dataflow, level_configs):
    ...
    #print(f'convert_args_and_invoke args shapelist:{shape_list},layer_type:{layer_type},num_pes:{num_pes},num_simd_lanes:{num_simd_lanes},bit_width:{bit_width},bandwitdh:{bandwidth},level_configs:{level_configs},buf_sizes:{buf_sizes},num_sub_clusters:{num_sub_clusters},dataflow_list:{dataflow_list},search_permutations:{args.search_permutations}')
    ...
    if cost['ExactRunTime'] <= 0 or cost['OverallEnergy'] <= 0 or cost['Area'] <= 0:
        ...
        # print(f'failure none 1 {cost["ExactRunTime"]},{cost["OverallEnergy"]},{cost["Area"]},{cost},elapse:{end - start},ret:{ret}')
        return None
...
def convert_args_and_invoke_gemm(args, eval_func, shape, num_simd_lanes, bit_width, bandwidth, dataflow, level_configs):
    global failure_stats

    assert(len(shape[1]) == 3 and len(shape[2]) == 3)
    if dataflow == 'searched':
        for level_config in level_configs:
            assert(len(level_config.tile_sizes) == 3)

    buf_sizes = [x.buf_size for x in level_configs]
    num_sub_clusters = [x.num_sub_clusters for x in level_configs]

    dataflow_list = list()
    tile_order_default = ['M', 'K', 'N']
    if dataflow == 'searched':
        for i, level_config in enumerate(level_configs):
            s_dim = level_config.spatial_dim
            tile_size = level_config.tile_sizes[s_dim]
        ...
        dataflow_list.append('SM|' + str(level_configs[0].tile_sizes['M']))
        dataflow_list.append('TN|' + str(level_configs[0].tile_sizes['N']))
        dataflow_list.append('TK|' + str(level_configs[0].tile_sizes['K']))
        dataflow_list.append('C')
        dataflow_list.append('TM|' + str(level_configs[1].tile_sizes['M']))
        dataflow_list.append('TN|' + str(level_configs[1].tile_sizes['N']))
        dataflow_list.append('SK|' + str(level_configs[1].tile_sizes['K']))
        args.search_permutations = False
    elif dataflow == 'shi':
        assert False
        args.search_permutations = False
    elif dataflow == 'dla':
        dataflow_list.append('SM|' + str(level_configs[0].tile_sizes['N']))
        dataflow_list.append('TN|' + str(level_configs[0].tile_sizes['K']))
        dataflow_list.append('TK|' + str(level_configs[0].tile_sizes['M']))
        dataflow_list.append('C')
        dataflow_list.append('TM|' + str(level_configs[1].tile_sizes['N']))
        dataflow_list.append('TN|' + str(level_configs[1].tile_sizes['K']))
        dataflow_list.append('SK|' + str(level_configs[1].tile_sizes['M']))
        args.search_permutations = False
    # shape_list = list(itertools.chain(*[(shape[1][x], shape[2][x]) for x in tile_order_default]))
    shape_list = list(itertools.chain([(shape[1][x]) for x in tile_order_default]))
    layer_type = shape[3]
    # TODO: DSCONV causes seg fault (likely because dataflow requirements are different)
    layer_type = 'GEMM'
    ...
    print(f'convert_args_and_invoke_gemm args shape:{shape},shapelist:{shape_list},layer_type:{layer_type},num_pes:{num_pes},num_simd_lanes:{num_simd_lanes},bit_width:{bit_width},bandwitdh:{bandwidth},level_configs:{level_configs},buf_sizes:{buf_sizes},num_sub_clusters:{num_sub_clusters},dataflow_list:{dataflow_list},search_permutations:{args.search_permutations},logpath:{logpath}')
    ...
    print(f'convert_args_and_invoke_gemm cost:{ret}')
    if args.dump_all:
        cost = json.loads(ret.decode('utf-8'))
        cost['Power'] = 0
    else:
