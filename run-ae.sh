...
help_single() {
    ...
    echo "  -h"
    echo "  --help        Print this message."
    echo "  --model       DL model. TRANSFORMER."
...
run_single() {
    ...
    case $technique in
        ...
        Eyeriss)
            ...
            if [[ $scale == "Edge" ]]; then
                extra_flags="--hw-point=eyeriss_edge --dataflow=fixed "
            else
                extra_flags="--hw-point=eyeriss_cloud --dataflow=fixed "
            fi
            ;;
        NVDLA)
            real_technique="bo_sw_search"
            ...
            runtime_limit="00:59:59"
            if [[ $scale == "Edge" ]]; then
                extra_flags="--hw-point=nvdla_edge --dataflow=fixed "
            else
                extra_flags="--hw-point=nvdla_cloud --dataflow=fixed "
            fi
            ;;
        MAERI)
            real_technique="bo_hw_sw_search"
            if [[ $scale == "Edge" ]]; then
                extra_flags=" --dataflow=fixed --simd-low=1 --simd-high=1 --prec-low=8 --prec-high=8 --bw-low=64 --bw-high=64 --l1-low=65 --l1-high=65 --l2-low=80 --l2-high=80 --pe-low=374 --pe-high=374"
            else
                extra_flags=" --dataflow=fixed --simd-low=1 --simd-high=1 --prec-low=8 --prec-high=8 --bw-low=64 --bw-high=64 --l1-low=1024 --l1-high=1024 --l2-low=2048 --l2-high=2048 --pe-low=14336 --pe-high=14336"
            fi
            ;;
        *)
    ...
    case $algorithm in
        Spotlight-GA)
            ...
            extra_flags="$extra_flags --hw-batch-size=20 --sw-batch-size=20 --scale-trials"
            ;;
        Spotlight-R)
            real_technique="random_hw_sw_search"
            extra_flags="$extra_flags --scale-trials"
            ;;
        Spotlight-V)
            real_technique="bo_hw_sw_search"
            extra_flags="$extra_flags --exclude-feat=original,intuitive,data-driven"
            ;;
#        Spotlight-F)
#            real_technique="bo_hw_sw_search"
...
#            ;;
        Spotlight)
            ...
            ;;
        Spotlight-HEBO)
            real_technique="hebo_hw_sw_search"
            ;;
        *)
            echo "Invalid technique: $technique"
            help_single
...
EOM
        ...
        python src/main.py $real_progress_bar --model=$real_technique --trials=$trials --layers=$model --target=$real_target --hw-trials=$hw_trials --sw-trials=$sw_trials $real_scale --output-dir=$result_dir --output-filename=out.txt --output-to-file $extra_flags --technique=$technique $@  > ./log/${real_technique}_out_$(date +%m%d%H%M).txt
...
if [[ $mode == "single" ]]; then
    ...
    run_single $technique $model $target $trials $scale 1 $algorithm $@
    ...
    for model in TRANSFORMER; do
        for target in EDP Delay; do
            for technique in Spotlight-HEBO Spotlight Eyeriss NVDLA MAERI; do
    ...
    for model in TRANSFORMER; do
        for target in EDP Delay; do
            for technique in Spotlight-HEBO Spotlight-GA Spotlight-R Spotlight-V Spotlight-F; do
                echo "$technique-$target-$model"
